...
if test -z "$PETSCREPO_BUILDS"; then
  PETSCREPO_BUILDS=ser,par
  if test -n "$BUILD_PETSCCOMPLEX"; then
    PETSCREPO_BUILDS=${PETSCREPO_BUILDS},sercplx,parcplx
  fi
  if $BUILD_DEBUG; then
    for build in `echo $PETSCREPO_BUILDS | tr , " "`; do
      PETSCREPO_BUILDS=$PETSCREPO_BUILDS,${build}dbg
    done
  fi
# Default is static but shared versions are needed for COSML
  if test -n "$BUILD_PETSCSHARED"; then
    for build in `echo $PETSCREPO_BUILDS | tr , " "`; do
      PETSCREPO_BUILDS=$PETSCREPO_BUILDS,${build}sh
    done
  fi
fi

PETSCREPO_DEPS=atlas,clapack_cmake,mercurial,hypre,superlu,superlu_dist,Python
BUILD_PETSC_WITH_GPU_CODE=${BUILD_PETSC_WITH_GPU_CODE:-false}
if $BUILD_PETSC_WITH_GPU_CODE; then
 # PETSCREPO_DEPS=$PETSCREPO_DEPS,cusp
 PETSCREPO_DEPS=cusp
fi
PETSCREPO_UMASK=002
...
get_petsc_env() {
  echo "$PETSC_ENV PETSC_DIR=$BUILD_DIR/petscrepo${PETSCDIR_SUFFIX} PETSC_ARCH=$1"
}

######################################################################
#
# Launch petscrepo builds.
#
######################################################################
...
buildPetscrepo() {
  ...
  #
  if test -d $PROJECT_DIR/petscrepo; then
    PETSCDIR_SUFFIX=""
    getVersion petscrepo
    # SEK: No preconfig needed in general but perhaps there is
    # something else in preconfig we need?
    bilderPreconfig petscrepo
    res=$?
    if $SVNUP; then
      cd $PROJECT_DIR/petscrepo
      hg pull
      cd -
    fi
  else
    PETSCDIR_SUFFIX="-${PETSCREPO_BLDRVERSION}"
    bilderUnpack petscrepo
    res=$?
  fi
  ###---------------------------------------------------------------------------
...
#        PETSC_PAR_FORTRAN_CONF="--with-fortran=1 --with-fc='$PF77'"
    ...
    for build in `echo $PETSCREPO_BUILDS | tr , " "`; do
      ...
      if bilderConfig -i $barg petscrepo $build "$config_args" "" "$PETSC_DIRARG"; then
         bilderBuild petscrepo $build "" "$PETSC_DIRARG"
      fi
    done
...
installPetscrepo() {
  ...
  for build in `echo $PETSCREPO_BUILDS | tr , " "`; do
    ...
    if test $build == "ser"; then
      ...
      lnpetscname="petsc3.3-$build"
    fi
    bilderInstall -r -b "$BUILD_DIR/petscrepo${PETSCDIR_SUFFIX}" petscrepo $build "$lnpetscname" "" "$PETSC_DIRARG"
